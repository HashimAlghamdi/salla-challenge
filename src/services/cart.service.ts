...
export const cartService = {
  getCart: async (): Promise<Cart> => {
    const response = await apiClient.get<Cart>(`/cart/?token=${localStorage.getItem('token')}`);
  ...
  updateCartItem: async (itemId: number, data: UpdateCartRequest): Promise<Cart> => {
  ...
  deleteCartItem: async (itemId: number): Promise<void> => {
    await apiClient.delete(`/cart/delete/${itemId}/?token=${localStorage.getItem('token')}`);
  },
};
